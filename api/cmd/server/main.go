...
var (
	...
	uiDistPath             = flag.String("ui.path", "dist", "path to dist assets dir")
...
func main() {
	...
	conn, err := dbr.Open("sqlite3", *dbPath, nil)
	if err != nil {
	...
	if !*migrationsDisabled {
		n, err := migrate.Exec(conn.DB, "sqlite3", &migrate.FileMigrationSource{Dir: *migrationsPath}, migrate.Up)
		if err != nil {
	...
	logger.Fatal("Server Exited", zap.Error(server.NewServer(config, logger, conn).Start()))
}
